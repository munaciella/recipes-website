export const copy = {
  common: {
    nav: {
      logo: {
        alt: "velovegans img",
        src: "/assets/velovegans-yellow.png",
        darkSrc:  "/assets/velovegans-white.png",
      },
      home: "Home",
      about: "About",
      recipes: "Recipes",
      contact: "Contact",
      signup: "Sign Up",
      login: "Login",
    },
    footer: {
      companyName: "VeloVegans.",
      msg: "All rights reserved.",
      socials: [
        { href: "https://www.instagram.com/velovegans/", name: "Instagram" },
        { href: "https://www.threads.net/@velovegans", name: "Threads" },
        { href: "https://x.com/velovegans", name: "X" },
      ],
    },
  },
  home: {
    description: {
      heading: {
        top: "VeloVegans",
        bottom: "Cruelty Free Recipes",
      },
      paragraph: "Saving the animals and our planet one meal at a time.",
      img: {
        alt: "velovegans logo",
        src: "/assets/cruelty-free-vegan.png",
      },
    },
  },
  contact: {
    contactDescription: {
      heading: "Get in Touch",
      button: "Work with me",
    },
  },
  about: {
    details: {
      heading: "About Us",
      paragraph:
        "Welcome to our vegan recipes website, where we celebrate compassion, health and sustainability. Our delicious, cruelty-free recipes promote a healthier, kinder lifestyle. Join us in making a positive impact on the planet with every meal. Click below to learn more about veganism and sustainability and how you can get involved.",
      usefulLinks: [
        {
          name: "Cowspiracy",
          href: "https://www.cowspiracy.com/",
          img: {
            src: "/assets/cowspiracy.png",
            alt: "Cowspiracy",
          }
        },
        {
          name: "What The Health",
          href: "https://www.whatthehealthfilm.com/",
          img: {
            src: "/assets/what_the_health.png",
            alt: "What The Health",
          }
        },
        {
          name: "This Is Vegan Propaganda",
          href: "https://earthlinged.org/",
          img: {
            src: "/assets/vegan-propaganda.png",
            alt: "This Is Vegan Propaganda",
          }
        },
        {
          name: "Forks Over Knives",
          href: "https://www.forksoverknives.com/",
          img: {
            src: "/assets/forks-over-knives.png",
            alt: "Forks Over Knives",
          }
        },
        {
          name: "The Game Changers",
          href: "https://www.thegamechangersmovie.com/",
          img: {
            src: "/assets/gamechangers.png",
            alt: "The Game Changers",
          }
        },
        {
          name: "Peta UK",
          href: "https://www.peta.org.uk/",
          img: {
            src: "/assets/petauk.png",
            alt: "Peta UK",
          }
        },
      ]
    },
  },
}
  